# E9T2 — Redis Integration

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CWB

Implement Redis for alert cooldown tracking, quiet hour state, and distributed job coordination. Add tests for Redis interactions
...
If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/E9T2.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.
