# E7T4 — Cooldown Enforcement

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CWB

Implement 4-hour cooldown period after alert fires. Add tests/specs/E7T4.json for cooldown behavior verification
...
If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/E7T4.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.
