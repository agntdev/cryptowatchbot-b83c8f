# E8T2 — Admin Statistics Commands

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CWB

Implement /admin stats command showing user counts, active alerts, and top-fired alerts. Add tests/specs/E8T2.json for metrics verification
...
If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/E8T2.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.
