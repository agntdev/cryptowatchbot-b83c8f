# E9T1 — PostgreSQL Schema Implementation

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CWB

Create tables for users, watchlists, alerts, price snapshots, notifications, and owner metrics. Add tests for schema validation
...
If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/E9T1.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.
