# E7T3 — Price Feed Retry Logic

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CWB

Add exponential backoff retry system for price fetch failures (3 attempts max). Add tests/specs/E7T3.json for error handling
...
If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/E7T3.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.
