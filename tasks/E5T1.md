# E5T1 — Summary Time Configuration

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CWB

Create /summary set command with time validation. Add tests/specs/E5T1.json and append /summary to tests/commands.json
...
If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/E5T1.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.
