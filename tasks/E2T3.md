# E2T3 — Manage Watchlist Screen

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CWB

Create /watchlist command with inline toggles to remove coins. Add tests/specs/E2T3.json and append /watchlist to tests/commands.json
...
If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/E2T3.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.
