# E3T3 — Alert Command Support

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CWB

Implement /alert add <symbol> <type> <value> command syntax. Add tests/specs/E3T3.json and append /alert to tests/commands.json
...
If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/E3T3.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.
